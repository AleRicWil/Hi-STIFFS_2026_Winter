...
bool sendDataWiFi() {
  if (!client.connect(host_ip, host_port)) {
    if (hasSerial) Serial.println("Packet transfer to host failed");
    return false;
  }

  // Calculate binary packet length: 1 byte ID + sensors * (4 ts + 4 raw1 + 4 raw2)
  size_t packet_len = 1 + NUM_SENSORS * 12;

  // Send HTTP POST headers for binary data
  client.println("POST /data HTTP/1.1");
  client.print("Host: ");
  client.println(host_ip);
  client.println("Content-Type: application/octet-stream");
  client.print("Content-Length: ");
  client.println(packet_len);
  client.println("Connection: close");
  client.println();

  // Send binary data: nano_id as uint8_t
  uint8_t nano_id = atoi(NANO_ID);
  client.write(nano_id);
  ...
  for (int i = 0; i < NUM_SENSORS; i++) {
    ...
    client.write((const uint8_t*)&ts_us, sizeof(uint32_t));
    client.write((const uint8_t*)&raw_values[i][0], sizeof(int32_t));
    client.write((const uint8_t*)&raw_values[i][1], sizeof(int32_t));
  }
    ...
    case HAS_SERIAL:  // Send data over serial port at all times
      ...
      switch (WiFiState) {
        case CONNECTED: // Connected, do not allow OTA updates and send datastream to host 
          if (checkDataReady()) sendDataWiFi(); sendDataSerial();
          monitorConnection();
          break;        
